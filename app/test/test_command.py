...
class TestCommandClass(unittest.TestCase):
    def test_vim_insert(self):
        ...
        buffer = (
            ...
            .set_data(bytes(data.message_without_command(), "ascii"))
            .buffer
        )
        ...
        self.assertTrue(
            decoded_msg == "deez nuts", "It's Joever. Buffer data isn't correct."
        )
    ...
    def test_system_command(self):
        ...
        buffer = (
            command.set_type(data.command).set_data(bytes(data.message, "ascii")).buffer
        )
        decoded_msg = buffer[1:].decode("ascii").replace("\x00", "")

        self.assertIs(buffer[0], 3, "It's so over, the type isn't correct.")
        self.assertTrue(
            decoded_msg == "silent !i3 workspace 69",
            "It's Joever. Buffer data isn't correct.",
        )
